...
//   }
    ...
    public ArrayList<RateAvg> parseRate(String jsonText){
        try {
            ...
            List<Map<String,Object>> list = (List<Map<String,Object>>)tasksListJson.get("AVG");
...
//                t.setId((int)value);
if (obj.containsKey("AVG(Rate)"))
//System.out.println(obj.get("AVG(Rate)").toString());
                
              t.setAvgRate(Float.parseFloat(obj.get("AVG(Rate)").toString()));
          //     t.setNbComment((int)Float.parseFloat(obj.get("count(comment)").toString()));
...
//      
    ...
    public ArrayList<RateAvg> findProd(int id){
        String url ="http://localhost/VeloSymfonyIntegre/Velo/web/app_dev.php/api/avg/"+id;
        req.setUrl(url);
        req.setPost(false);
        req.addResponseListener(new ActionListener<NetworkEvent>() {
            @Override
